jQuery.sap.declare("SelfServicePortal.util.Grouper");

SelfServicePortal.util.Grouper = {

	bundle : null, // somebody has to set this

	LifecycleStatus : function (oContext) {
		var status = oContext.getProperty("LifecycleStatus");
		var text = SelfServicePortal.util.Grouper.bundle.getText("StatusText" + status, "?");
		return {
			key: status,
			text: text
		};
